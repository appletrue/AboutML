...
From Wiki & Blogs
### 样本空间
随机试验E的所有结果构成的集合称为E的 样本空间，记为S={e},称S中的元素e为样本点，一个元素的单点集称为基本事件。
...
联合概率：表示两个事件共同发生的概率 P(A∩B)

边缘概率：边缘概率是某个事件发生的概率。A的边缘概率表示为P（A），B的边缘概率表示为P（B）。

在联合概率中，把最终结果中不需要的那些事件合并成其事件的全概率而消失（对离散随机变量用求和得全概率，对连续随机变量用积分得全概率）。这称为边缘化（marginalization）。
...
$ Pr(A)= sum_{n}Pr(A\mid B_{n})Pr(B_{n})$,
...
在离散情况下，上述公式等于下面这个公式：$Pr(A)=E(Pr(A\mid N))$,但后者在连续情况下仍然成立：此处N是任意随机变量。这个公式还可以表达为："A的先验概率等于A的后验概率的先验期望值。
...
一批产品共8件，其中正品6件，次品2件．现不放回地从中取产品两次，每次一件，求第2次取得正品的概率．
...
在全概率公式中，如果将A看成是“结果”，Bi看成是导致结果发生的诸多“原因”之一，那么**全概率公式就是一个原因推结果的过程**

贝叶斯公式中，是知道结果A已经发生，反过来研究造成结果发生的原因，是XX原因造成的可能性有多大，即“结果推原因”。
...
按以上术语，Bayes定理可表述为：__后验概率 = (相似度X先验概率)/标准化常量__，也就是說，后验概率与先验概率和相似度的乘积成正比。另外，比例P(B|A)/P(B)也有时被称作标准相似度（standardised likelihood），Bayes定理可表述为：后验概率 = 标准相似度X先验概率。
...
$P(A|B)P(B)=P(A \cap B)=P(B|A)P(A)$ (概率乘法法则).

上式两边除以P(B), 若P(B) 非0 ,则可得贝叶斯定理: $P(A|B)= \frac{P(B|A)P(A)}{P(B)}$
...
通过知道一个人的身高、体重以及脚的尺寸，去判断这个人是男是女。
